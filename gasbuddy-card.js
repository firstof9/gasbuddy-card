...
`;function ue(e){if(null==e||"unknown"===e||"unavailable"===e)return"-";const t=Number(e);return isNaN(t)?String(e):new Intl.NumberFormat(void 0,{style:"currency",currency:"USD",minimumFractionDigits:2,maximumFractionDigits:3}).format(t)}function ve(e){if(!e||"unknown"===e||"unavailable"===e)return"Unknown";try{const t=new Date(String(e));if(isNaN(t.getTime()))return String(e);const s=(new Date).getTime()-t.getTime(),i=Math.floor(s/6e4),a=Math.floor(i/60);return i<1?"Just now":i<60?`${i}m ago`:a<24?`${a}h ago`:t.toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return String(e)}}const ge=[{name:"device_id",label:"GasBuddy Station / Device",helper:"Select the GasBuddy station device configured in your Home Assistant.",selector:{device:{integration:"gasbuddy"}}},{name:"title",label:"Card Title (Optional)",helper:"Custom title shown at the top of the card. Falls back to the station name.",selector:{text:{}}},{name:"regular_gas_entity",label:"Regular Gas Sensor Override",selector:{entity:{domain:"sensor"}}},{name:"premium_gas_entity",label:"Premium Gas Sensor Override",selector:{entity:{domain:"sensor"}}},{name:"diesel_entity",label:"Diesel Sensor Override",selector:{entity:{domain:"sensor"}}},{name:"ev_dc_fast_entity",label:"EV DC Fast Chargers Override",selector:{entity:{domain:"sensor"}}},{name:"ev_level2_entity",label:"EV Level 2 Chargers Override",selector:{entity:{domain:"sensor"}}}];let fe=class extends ne{setConfig(e){this._config=e}render(){return this.hass&&this._config?F`
      ...
      ></ha-form>
    `:F``}_valueChanged(e){this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e.detail.value},bubbles:!0,composed:!0}))}};e([he({attribute:!1})],fe.prototype,"hass",void 0),e([_e()],fe.prototype,"_config",void 0),fe=e([ce("gasbuddy-card-editor")],fe);let me=class extends ne{constructor(){super(...arguments),this._activeTab="gas"}static{this.styles=pe}setConfig(e){if(!e)throw new Error("Invalid configuration");this._config=e,e.default_mode&&(this._activeTab=e.default_mode)}getCardSize(){return 4}static getConfigElement(){return document.createElement("gasbuddy-card-editor")}static getStubConfig(e,t){const s=t.find(t=>{const s=e.states[t];return t.startsWith("sensor.")&&s&&s.attributes&&(s.attributes.attribution?.toLowerCase().includes("gasbuddy")||t.toLowerCase().includes("gasbuddy"))});let i="";return s&&e.entities&&e.entities[s]&&(i=e.entities[s].device_id||""),{type:"custom:gasbuddy-card",device_id:i,default_mode:"gas"}}render(){if(!this.hass||!this._config)return F``;const e=this._config.device_id;if(!e)return F`
        <ha-card>
      ...
      <ha-card>
        ...
        <div class="header">
          ...
          <div class="brand-logo">
            ${"ev"===n&&s.ev_network?function(e){if(!e)return F`<ha-icon icon="mdi:ev-station"></ha-icon>`;const t=e.toLowerCase();return t.includes("tesla")?F`
    ...
    `:t.includes("electrify america")?F`
    ...
    `:F`<ha-icon icon="mdi:ev-station"></ha-icon>`}(this.hass.states[s.ev_network]?.state||""):_?F`<img src="${_}" alt="Brand logo" />`:F`<ha-icon icon="mdi:gas-station"></ha-icon>`}
          </div>
        </div>
    ...
    `}_renderGasContent(e){const t=[{key:"regular_gas",name:"Regular",cashKey:"regular_gas_cash"},{key:"midgrade_gas",name:"Midgrade",cashKey:"midgrade_gas_cash"},{key:"premium_gas",name:"Premium",cashKey:"premium_gas_cash"},{key:"diesel",name:"Diesel",cashKey:"diesel_cash"},{key:"e15",name:"UNL88",cashKey:"e15_cash"},{key:"e85",name:"E85",cashKey:"e85_cash"}].filter(t=>{const s=t.key in e&&e[t.key]?this.hass.states[e[t.key]]:void 0,i=t.cashKey in e&&e[t.cashKey]?this.hass.states[e[t.cashKey]]:void 0,a=e=>e&&"unavailable"!==e.state&&"unknown"!==e.state;return a(s)||a(i)});return F`
      <div class="gas-grid">
        ${t.map(t=>{const s=e[t.key],i=e[t.cashKey],a=s?this.hass.states[s]:void 0,r=i?this.hass.states[i]:void 0,n=a?ue(a.state):"",o=r?ue(r.state):"",c=n||o||"-",l=n&&o&&n!==o;let d="";return a&&a.attributes.unit_of_measurement?d=a.attributes.unit_of_measurement:r&&r.attributes.unit_of_measurement&&(d=r.attributes.unit_of_measurement),F`
    ...
    `}_renderEVContent(e){const t=e.ev_level1,s=e.ev_level2,i=e.ev_dc_fast,a=t&&Number(this.hass.states[t]?.state)||0,r=s&&Number(this.hass.states[s]?.state)||0,n=i&&Number(this.hass.states[i]?.state)||0,o=[{name:"J1772",countId:e.ev_j1772,powerId:e.ev_j1772_power},{name:"CCS",countId:e.ev_ccs,powerId:e.ev_ccs_power},{name:"CHAdeMO",countId:e.ev_chademo,powerId:e.ev_chademo_power},{name:"NACS",countId:e.ev_nacs,powerId:e.ev_nacs_power}].filter(e=>e.countId&&"unavailable"!==this.hass.states[e.countId]?.state&&Number(this.hass.states[e.countId]?.state)>0),c=e.ev_network?this.hass.states[e.ev_network]?.state:"",l=e.ev_network?this.hass.states[e.ev_network]:void 0,d=l&&l.attributes?l.attributes.website:void 0,h=e.ev_pricing?this.hass.states[e.ev_pricing]?.state:"",_=e.ev_access_hours?this.hass.states[e.ev_access_hours]?.state:"",p=e.ev_cards_accepted?this.hass.states[e.ev_cards_accepted]?.state:"",u=e.ev_status?this.hass.states[e.ev_status]?.state:"",v=e.ev_date_last_confirmed?this.hass.states[e.ev_date_last_confirmed]?.state:"";return F`
        ...
        ${o.length>0?F`
              <div>
                ...
                <div class="connectors-grid">
                  ${o.map(e=>{const t=this.hass.states[e.countId]?.state||"0",s=e.powerId?this.hass.states[e.powerId]?.state:void 0;return F`
                      <div class="connector-card" style="border-color: rgba(var(--rgb-primary-color, 33, 150, 243), 0.2);">
                        ...
                        <div class="connector-details">
                          ...
                          ${s&&"unknown"!==s&&"unavailable"!==s?F`<span class="connector-power">${s} kW</span>`:""}
                        </div>
                      </div>
                    `})}
                </div>
        ...
        <div class="metadata-list">
          ${c&&"unknown"!==c&&"unavailable"!==c?F`
                <div class="metadata-item">
                  ...
                  >
                    ${d?F`<a href="${d}" target="_blank" rel="noopener noreferrer">${c}</a>`:c}
                  </span>
                </div>
              `:""}
          ${u&&"unknown"!==u&&"unavailable"!==u?F`
                <div class="metadata-item">
                  <span class="metadata-key">Status</span>
                  <span class="metadata-val">${String(u).toUpperCase()}</span>
                </div>
              `:""}
          ${h&&"unknown"!==h&&"unavailable"!==h?F`
                <div class="metadata-item">
                  <span class="metadata-key">Pricing</span>
                  <span class="metadata-val">${h}</span>
                </div>
              `:""}
          ${_&&"unknown"!==_&&"unavailable"!==_?F`
                <div class="metadata-item">
                  <span class="metadata-key">Access Hours</span>
                  <span class="metadata-val">${_}</span>
                </div>
              `:""}
          ${p&&"unknown"!==p&&"unavailable"!==p?F`
                <div class="metadata-item">
                  <span class="metadata-key">Payments</span>
                  <span class="metadata-val">
                    ${(()=>{const e=function(e){if(!e)return[];const t=e.toLowerCase().split(/[\s,;]+/).map(e=>e.trim()).filter(Boolean),s=[],i=new Set;for(const e of t)"v"!==e&&!e.includes("visa")||i.has("visa")?"m"!==e&&!e.includes("mastercard")&&!e.includes("master")||i.has("mastercard")?("a"===e||e.includes("american")||e.includes("express")||e.includes("amex"))&&!i.has("amex")?(s.push(F`
      ...
      `),i.add("amex")):"d"!==e&&!e.includes("discover")||i.has("discover")?e.includes("debit")&&!i.has("debit")&&(s.push(F`
      ...
      `),i.add("visa"));return s}(String(p));return e.length>0?F`<div class="payment-icons-container">${e}</div>`:p})()}
                  </span>
                </div>
              `:""}
          ${v&&"unknown"!==v&&"unavailable"!==v?F`
                <div class="metadata-item">
    ...
    `}};e([he({attribute:!1})],me.prototype,"hass",void 0),e([_e()],me.prototype,"_config",void 0),e([_e()],me.prototype,"_activeTab",void 0),me=e([ce("gasbuddy-card")],me),window.customCards=window.customCards||[],window.customCards.push({type:"gasbuddy-card",name:"GasBuddy Card",preview:!0,description:"A premium Home Assistant custom card for GasBuddy integration gas prices and EV stations."});export{me as GasBuddyCard};
